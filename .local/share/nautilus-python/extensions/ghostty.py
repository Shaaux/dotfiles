...
# Modified to support opening new tmux windows in existing session
#
...
# Configuration: tmux session name to use
...
class OpenInGhosttyAction(GObject.GObject, Nautilus.MenuProvider):
    ...
    def _open_terminal(self, path):
        ...
        if has_session:
            ...
            # Check if any clients are already attached (ghostty already open)
            clients = subprocess.run(
                ['tmux', 'list-clients', '-t', TMUX_SESSION_NAME],
                capture_output=True, text=True
            )
            if clients.stdout.strip():
                # Client already attached - don't open new ghostty window
                return
        
        # No session OR no attached clients - open ghostty
